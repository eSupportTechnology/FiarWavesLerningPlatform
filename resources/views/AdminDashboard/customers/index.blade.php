                    ...
                    <div class="row g-2 align-items-center">
                        <div class="col-md-4">
                            <input type="text" name="search" class="form-control"
                                placeholder="Search by name, email, contact or invite code" value="{{ request('search') }}">
                        </div>
                                    ...
                                    <td class="d-flex flex-wrap justify-content-center gap-2">
                                        ...
                                        @if ($loggedInEmployee && $loggedInEmployee->role === 'super_admin')
                                            ...
                                            <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal"
                                                data-bs-target="#editCustomerModal{{ $customer->user_id }}">
                                                Edit
                                            </button>
                                        @endif

                                        <!-- Edit Customer Modal -->
                                        <div class="modal fade" id="editCustomerModal{{ $customer->user_id }}"
                                            tabindex="-1" aria-labelledby="editCustomerModalLabel{{ $customer->user_id }}"
                                            aria-hidden="true">
                                            <div class="modal-dialog modal-lg">
                                                <div class="modal-content">
                                                    <form
                                                        action="{{ route('admin.customers.update', $customer->user_id) }}"
                                                        method="POST" enctype="multipart/form-data">
                                                        @csrf
                                                        @method('PUT')
                                                        <div class="modal-header">
                                                            <h5 class="modal-title"
                                                                id="editCustomerModalLabel{{ $customer->user_id }}">Edit
                                                                Customer</h5>
                                                        ...
                                                        <div class="modal-body row g-3">
                                                            ...
                                                            <div class="col-md-6">
                                                                ...
                                                                <input type="file" name="bank_front_image"
                                                                    class="form-control">
                                                            </div>
                                                            <div class="col-md-6">
                                                                <label class="form-label">Bank Back Image</label>
                                                                <input type="file" name="bank_back_image"
                                                                    class="form-control">
                                                            </div>
                                                            <div class="col-md-6">
                                                                ...
                                                                <select name="bank_status" class="form-select">
                                                                    <option value=""
                                                                        {{ is_null($customer->bank_status) ? 'selected' : '' }}>
                                                                        Not Applied</option>
                                                                    <option value="approved"
                                                                        {{ $customer->bank_status == 'approved' ? 'selected' : '' }}>
                                                                        Approved</option>
                                                                    <option value="rejected"
                                                                        {{ $customer->bank_status == 'rejected' ? 'selected' : '' }}>
                                                                        Rejected</option>
                                                                    <option value="pending"
                                                                        {{ $customer->bank_status == 'pending' ? 'selected' : '' }}>
                                                                        Pending</option>
                                                                </select>
                                                            </div>
                                                            <div class="col-md-6">
                                                                ...
                                                                <select name="kyc_doc_type" class="form-select">
                                                                    <option value=""
                                                                        {{ is_null($customer->kyc_doc_type) ? 'selected' : '' }}>
                                                                        Select</option>
                                                                    <option value="NIC"
                                                                        {{ $customer->kyc_doc_type == 'NIC' ? 'selected' : '' }}>
                                                                        NIC</option>
                                                                    <option value="DL"
                                                                        {{ $customer->kyc_doc_type == 'DL' ? 'selected' : '' }}>
                                                                        DL</option>
                                                                    <option value="Passport"
                                                                        {{ $customer->kyc_doc_type == 'Passport' ? 'selected' : '' }}>
                                                                        Passport</option>
                                                                </select>
                                                            </div>
                                                            <div class="col-md-6">
                                                                <label class="form-label">KYC Document Number</label>
                                                                <input type="text" name="kyc_doc_number"
                                                                    class="form-control"
                                                                    value="{{ $customer->kyc_doc_number }}">
                                                            </div>
                                                            <div class="col-md-6">
                                                                <label class="form-label">KYC Front Image</label>
                                                                <input type="file" name="kyc_doc_front"
                                                                    class="form-control">
                                                            </div>
                                                            <div class="col-md-6">
                                                                <label class="form-label">KYC Back Image</label>
                                                                <input type="file" name="kyc_doc_back"
                                                                    class="form-control">
                                                            </div>
                                                            <div class="col-md-6">
                                                                ...
                                                                <select name="kyc_status" class="form-select">
                                                                    <option value=""
                                                                        {{ is_null($customer->kyc_status) ? 'selected' : '' }}>
                                                                        Not Applied</option>
                                                                    <option value="approved"
                                                                        {{ $customer->kyc_status == 'approved' ? 'selected' : '' }}>
                                                                        Approved</option>
                                                                    <option value="rejected"
                                                                        {{ $customer->kyc_status == 'rejected' ? 'selected' : '' }}>
                                                                        Rejected</option>
                                                                    <option value="pending"
                                                                        {{ $customer->kyc_status == 'pending' ? 'selected' : '' }}>
                                                                        Pending</option>
                                                                </select>
                                                            </div>
                                                            <div class="col-md-6">
                                                                ...
                                                                <select name="status" class="form-select">
                                                                    <option value="1"
                                                                        {{ $customer->status ? 'selected' : '' }}>Active
                                                                    </option>
                                                                    <option value="0"
                                                                        {{ !$customer->status ? 'selected' : '' }}>Inactive
                                                                    </option>
                                                                </select>
                                                            </div>
                                                            <div class="col-md-6">
                                                                ...
                                                                <select name="user_type" class="form-select">
                                                                    <option value="user"
                                                                        {{ $customer->user_type == 'user' ? 'selected' : '' }}>
                                                                        User</option>
                                                                    <option value="super_user"
                                                                        {{ $customer->user_type == 'super_user' ? 'selected' : '' }}>
                                                                        Super User</option>
                                                                </select>
                                                            </div>
                        ...
                        <div>
                            <small>
                                ...
                                @if ($customers->total() > 0)
                                    {{ $customers->firstItem() }} to {{ $customers->lastItem() }} of
