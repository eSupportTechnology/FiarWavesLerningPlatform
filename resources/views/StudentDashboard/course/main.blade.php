...
@section('content')
    <div class="container py-4">
        ...
        <form method="GET" action="{{ route('student.bookings') }}" class="mb-4">
            <div class="row g-2 align-items-center">
                <div class="col-md-4">
                    <input type="text" name="search" class="form-control" placeholder="Search courses by name"
                        value="{{ request('search') }}">
                </div>
                <div class="col-md-auto">
                    <button type="submit" class="btn btn-primary">Search</button>
                    <a href="{{ route('student.bookings') }}" class="btn btn-secondary">Reset</a>
                </div>
            </div>
        </form>
                    ...
                    <div class="card shadow-sm border-0 h-100">
                        ...
                        <div class="card-body d-flex flex-column">
                            ...
                            <div class="mt-auto d-flex flex-wrap gap-2">
                                <a href="{{ route('student.course.details', $booking->id) }}"
                                    class="btn btn-primary btn-sm">
                                    <i class="bi bi-info-circle"></i> View More
                                </a>

                                @if ($booking->course->files->count())
                                    <a href="{{ route('student.course.files', $booking->id) }}"
                                ...
                                @if ($booking->course->recordings->count())
                                    <a href="{{ route('student.course.recordings', $booking->id) }}"
                                ...
                                @if ($booking->course->zoomLinks->count())
                                    <a href="{{ route('student.course.zoom', $booking->id) }}"
